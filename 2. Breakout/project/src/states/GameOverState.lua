...
function GameOverState:update(dt)
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        ...
        if highScore and not hasCheated then
...
function GameOverState:render()
    ...
    if hasCheated then
      love.graphics.printf('Cheaters cannot enter highscores!', 0, VIRTUAL_HEIGHT / 2 + 20,
          VIRTUAL_WIDTH, 'center')
    end
    love.graphics.printf('Press Enter to continue', 0, VIRTUAL_HEIGHT - VIRTUAL_HEIGHT / 4,
        VIRTUAL_WIDTH, 'center')
end