...
function StartState:update(dt)
    ...
    if not self.pauseInput then
        ...
        -- if over start, self.currentMenuItem = 1
          -- if click, transition to next state
        
        if mouseX >= VIRTUAL_WIDTH/2 - gFonts['medium']:getWidth('Start')/2 and mouseX <= VIRTUAL_WIDTH/2 + gFonts['medium']:getWidth('Start')/2 and
        mouseY >= VIRTUAL_HEIGHT/2 + 20 and mouseY <= VIRTUAL_HEIGHT/2 + 20 + gFonts['medium']:getHeight('Start') then
          self.currentMenuItem = 1
          self.mouseInMenu = true
        elseif mouseX >= VIRTUAL_WIDTH/2 - gFonts['medium']:getWidth('Quit Game')/2 and mouseX <= VIRTUAL_WIDTH/2 + gFonts['medium']:getWidth('Quit Game')/2 and
        mouseY >= VIRTUAL_HEIGHT/2 + 45 and mouseY <= VIRTUAL_HEIGHT/2 + 45 + gFonts['medium']:getHeight('Quit Game') then
        ...
        -- if over exit, self.currentMenuItem = 0
          -- if click, quit game
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or (love.mouse.wasPressed(1) and self.mouseInMenu) then
