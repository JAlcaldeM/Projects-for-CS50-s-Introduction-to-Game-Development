...
public class LevelGenerator : MonoBehaviour {
    ...
    // positions where we won't place floor tiles
    private HashSet<Vector2Int> holes = new HashSet<Vector2Int>();

    // Use this for initialization
    void Start () {
        
        roomNumberText.text = GlobalVariables.roomNumber.ToString();
        ...
        for (int z = 0; z < mazeSize; z++) {
            for (int x = 0; x < mazeSize; x++) {
                ...
                // create floor and ceiling, unless it's a hole
    ...
    bool[,] GenerateMazeData() {
        ...
        // Select random positions for the holes
        ...
        while (tilesConsumed < tilesToRemove) {
            ...
            for (int i = 0; i < numSpacesMove; i++) {
                ...
                if (data[mazeY, mazeX]) {
                    ...
                    tilesConsumed++;
                    // randomly select three positions to create holes
                    if (holePositions.Count < 3 && Random.value < 0.05) {
                        holePositions.Add(new Vector2Int(mazeX, mazeY));
                    }
                }
            }
        }

        // Ensure we have exactly 3 holes
        while (holePositions.Count < 3) {
            int x = Random.Range(1, mazeSize - 1);
            int y = Random.Range(1, mazeSize - 1);
            var pos = new Vector2Int(x, y);
            if (!holePositions.Contains(pos) && !data[y, x]) {
                holePositions.Add(pos);
            }
        }

        // Assign the holes to the global holes set
        holes = new HashSet<Vector2Int>(holePositions);
