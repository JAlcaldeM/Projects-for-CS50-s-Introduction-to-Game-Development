...
function Player:checkRightCollisions(dt)
    ...
    if (tileTopRight and tileBottomRight) and (tileTopRight:collidable() or tileBottomRight:collidable()) then
        self.x = (tileTopRight.x - 1) * TILE_SIZE - self.width + 0.001
        self.movespeed = 0
    ...
    for k, object in pairs(self.level.objects) do
        if object:collides(self) then
            if object.solid then
                ...
                if not (object.y + 5 > self.y + self.height or self.y + 5> object.y + object.height) then
                  self.movespeed = 0
                end
